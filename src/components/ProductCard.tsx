...
export default function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <article className="group relative flex flex-col rounded-3xl border border-forest/8 bg-mint/40 p-3 transition-all duration-300 hover:-translate-y-1.5 hover:border-forest/15 hover:bg-white hover:shadow-lift">
      <Link to={`/product/${product.id}`} className="relative block aspect-square overflow-hidden rounded-2xl bg-mint-deep">
        <img
          src={product.img}
          alt={product.name}
      ...
      <button
        ...
        className={`absolute right-3 top-3 flex h-8 w-8 items-center justify-center rounded-full shadow-sm transition-all duration-200 hover:scale-110 active:scale-90 ${
      ...
      <button
        type="button"
        onClick={onAdd}
        className={`absolute inset-x-3 bottom-3 flex translate-y-2 items-center justify-center gap-1.5 rounded-full py-2.5 text-xs font-semibold text-white opacity-0 transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100 active:scale-95 ${
